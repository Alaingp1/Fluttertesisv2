...
class _OrdenarProductoState extends State<OrdenarProducto> {
  ...
  void generarOrden() async {
    ...
    print(producto);
    var url = "http://152.173.202.192/pruebastesis/generarOrden2.php";
  ...
  void validarDatos() async {
    var id = await FlutterSession().get('id');
    var url =
        "http://152.173.202.192/pruebastesis/validarDatos.php?Usuario_id=$id";
  ...
  Future obtenerCantidad() async {
    ...
    var url =
        "http://152.173.202.192/pruebastesis/obtenerCantidad.php?Producto_id=$idProd";
    final response = await http.get(Uri.parse(url));
